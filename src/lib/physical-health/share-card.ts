export interface PhysicalSharePayload {
  brand: string;
  title: string;
  profileLabel: string;
  scoreLabel: string;
...
export async function renderPhysicalSharePng(
  payload: PhysicalSharePayload,
): Promise<Blob> {
  const width = 1080;
  const height = 1400;
...
export async function downloadPhysicalCard(
  payload: PhysicalSharePayload,
  filename = "dailylogic-physical-health.png",
) {
  const blob = await renderPhysicalSharePng(payload);
...
export async function copyPhysicalCardImage(
  payload: PhysicalSharePayload,
): Promise<boolean> {
  const blob = await renderPhysicalSharePng(payload);
