export interface MentalSharePayload {
  brand: string;
  title: string;
  profileLabel: string;
  scoreLabel: string;
...
export async function renderMentalSharePng(
  payload: MentalSharePayload,
): Promise<Blob> {
  const width = 1080;
  const height = 1400;
...
export async function downloadMentalCard(
  payload: MentalSharePayload,
  filename = "dailylogic-mental-health.png",
) {
  const blob = await renderMentalSharePng(payload);
...
export async function copyMentalCardImage(
  payload: MentalSharePayload,
): Promise<boolean> {
  const blob = await renderMentalSharePng(payload);
