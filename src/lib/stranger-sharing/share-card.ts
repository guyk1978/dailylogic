export interface StrangerSharePayload {
  brand: string;
  title: string;
  profileLabel: string;
  scoreLabel: string;
...
export async function renderStrangerSharePng(
  payload: StrangerSharePayload,
): Promise<Blob> {
  const width = 1080;
  const height = 1400;
...
export async function downloadStrangerCard(
  payload: StrangerSharePayload,
  filename = "dailylogic-stranger-sharing.png",
) {
  const blob = await renderStrangerSharePng(payload);
...
export async function copyStrangerCardImage(
  payload: StrangerSharePayload,
): Promise<boolean> {
  const blob = await renderStrangerSharePng(payload);
