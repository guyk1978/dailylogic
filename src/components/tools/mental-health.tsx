...
const DEFAULT_SETUP: Partial<MentalSetup> = {};
...
function questionPrompt(
  ...
  if (periodAware) {
    const key = `questions.${questionId}.byPeriod.${setup.period}`;
    const tailored = t(key);
    if (tailored !== key) return tailored;
  }
  if (fatigueAware) {
    const key = `questions.${questionId}.byFatigue.${setup.fatigue}`;
    const tailored = t(key);
    if (tailored !== key) return tailored;
  }
  if (focusAware) {
    const key = `questions.${questionId}.byFocus.${setup.focus}`;
    const tailored = t(key);
    if (tailored !== key) return tailored;
  }
...
export function MentalHealthCalculator() {
  const { t } = useToolTranslation("mental-health-calculator");
  ...
  const [setup, setSetup] = useLocalStorage<Partial<MentalSetup>>(
    "tool:mental-health:setup",
    DEFAULT_SETUP,
  );
  const [mode, setMode] = useLocalStorage<MentalMode>(
    "tool:mental-health:mode",
    "quick",
  );
  const [answers, setAnswers] = useLocalStorage<MentalAnswers>(
    "tool:mental-health:answers",
    {},
  );
  ...
  const questions = useMemo(() => getMentalQuestionsForMode(mode), [mode]);
  const current = questions[index];
  const total = questions.length;
  const quizProgress = total > 0 ? Math.round(((index + 1) / total) * 100) : 0;
  const brandName = tCommon("app.name");
  const setupReady = isMentalSetupComplete(setup);
  ...
  const setupLine = useMemo(() => {
    ...
    return [
      t(`setup.period.${setup.period}`),
      t(`setup.fatigue.${setup.fatigue}`),
      t(`setup.focus.${setup.focus}`),
    ].join(" · ");
  }, [setup, setupReady, t]);

  const handleRestore = useCallback(
    (entry: CalculationHistoryEntry) => {
      const restoredSetup = (entry.inputs.setup as MentalSetup) ?? null;
      const restoredMode =
        (entry.inputs.mode as MentalMode) === "full" ? "full" : "quick";
      const restoredAnswers = (entry.inputs.answers as MentalAnswers) ?? {};
      if (restoredSetup && isMentalSetupComplete(restoredSetup)) {
      ...
      if (restoredSetup && isMentalSetupComplete(restoredSetup)) {
        const analyzed = analyzeMentalHealth(
          restoredMode,
          restoredSetup,
          restoredAnswers,
        );
  ...
  useCalculationRestore("mental-health-calculator", handleRestore);

  const { trackStart, trackAnswer, trackComplete, trackSetupChoice } =
    usePrivateToolStats("mental-health-calculator");

  const patchSetup = <K extends keyof MentalSetup>(
    key: K,
    value: MentalSetup[K],
  ) => {
  ...
  const startQuiz = (nextMode: MentalMode) => {
    if (!setupReady) return;
    setMode(nextMode);
    setAnswers({});
    setProfile(null);
  ...
  const goNext = () => {
    ...
    if (index >= total - 1) {
      const analyzed = analyzeMentalHealth(mode, setup, {
        ...answers,
  ...
  const handleDownloadCard = async () => {
    ...
    setCardBusy(true);
    try {
      await downloadMentalCard(sharePayload);
  ...
  const handleCopyCard = async () => {
    ...
    setCardBusy(true);
    try {
      const ok = await copyMentalCardImage(sharePayload);
  ...
  return (
    <div className="space-y-6">
      <div className="rounded-2xl bg-gradient-to-br from-emerald-50 via-white to-sky-50 p-5 shadow-md ring-1 ring-emerald-100/80 sm:p-6">
        <p className="label-caption mb-2 text-emerald-700/80">{t("intro.eyebrow")}</p>
      ...
      <AnimatePresence mode="wait">
        {phase === "setup" && (
          ...
          >
            <div className="rounded-2xl bg-white p-5 shadow-md sm:p-6">
              ...
              <SetupGroup label={t("setup.periodLabel")}>
                {(["highPressure", "routine", "transition"] as PeriodLoad[]).map(
                  (value) => (
                    <ChoiceChip
                      key={value}
                      selected={setup.period === value}
                      onClick={() => patchSetup("period", value)}
                      label={t(`setup.period.${value}`)}
                    />
                  ),
                )}
              </SetupGroup>

              <SetupGroup label={t("setup.fatigueLabel")}>
                {(["rested", "midweek", "wornDown"] as FatigueLevel[]).map(
                  (value) => (
                    <ChoiceChip
                      key={value}
                      selected={setup.fatigue === value}
                      onClick={() => patchSetup("fatigue", value)}
                      label={t(`setup.fatigue.${value}`)}
                    />
                  ),
                )}
              </SetupGroup>

              <SetupGroup label={t("setup.focusLabel")}>
                {(
                  [
                    "timePressure",
                    "futureWorry",
                  ...
                  <ChoiceChip
                    key={value}
                    selected={setup.focus === value}
                    onClick={() => patchSetup("focus", value)}
                    label={t(`setup.focus.${value}`)}
                  />
                ))}
              </SetupGroup>
            </div>
          ...
          >
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              <ModeCard
                ...
                icon={<HeartPulse className="h-5 w-5" strokeWidth={ICON_STROKE_WIDTH} />}
              />
              <ModeCard
                title={t("modes.full")}
        ...
        {phase === "result" && profile && (
          <motion.div
            ...
            className="space-y-4"
          >
            <div className="rounded-2xl border-2 border-emerald-100 bg-gradient-to-br from-sky-50/80 via-white to-teal-50 px-6 py-8 text-center shadow-md">
            ...
            <div className="rounded-2xl bg-white p-5 shadow-md ring-1 ring-slate-100">
              ...
              <div className="rounded-2xl bg-gradient-to-br from-emerald-50 via-white to-sky-50 p-5 ring-1 ring-emerald-100">
                ...
                <p className="mt-3 font-mono text-4xl font-bold text-emerald-800">
                  {formatScore(profile.overall)}%
                </p>
            ...
            <CalculationSavePanel
              toolSlug="mental-health-calculator"
              saveName={saveName}
