export interface BusinessSharePayload {
  brand: string;
  title: string;
  profileLabel: string;
  scoreLabel: string;
...
export async function renderBusinessSharePng(
  payload: BusinessSharePayload,
): Promise<Blob> {
  const width = 1080;
  const height = 1450;
  ...
  ctx.font = "500 26px system-ui, Segoe UI, Arial";
  for (const line of wrapText(ctx, payload.setupLine, width - 240).slice(0, 2)) {
    ctx.fillText(line, edge, y + 18);
    y += 36;
  }

  ctx.fillStyle = "#1d4ed8";
  ctx.font = "600 28px system-ui, Segoe UI, Arial";
  ctx.fillText(payload.profileLabel, edge, y + 40);
  ctx.font = "700 26px system-ui, Segoe UI, Arial";
  ctx.fillText(payload.scoreLabel, edge, y + 90);
  ctx.font = "800 108px ui-monospace, SFMono-Regular, Menlo, monospace";
  ctx.fillText(payload.scoreText, edge, y + 210);
...
export async function downloadBusinessCard(
  payload: BusinessSharePayload,
  filename = "dailylogic-business-partnership.png",
): Promise<void> {
  const blob = await renderBusinessSharePng(payload);
...
export async function copyBusinessCardImage(
  payload: BusinessSharePayload,
): Promise<boolean> {
